...
// global environment and class_table
...
CgenClassTable::CgenClassTable(Classes classes, ostream &s) : nds(NULL), str(s), labelid_(0), curr_cgenclass_(nullptr) {
...
void CgenClassTable::install_classes(Classes classes) {
    for (int i = classes->first(); classes->more(i); i = classes->next(i))
        install_class(new CgenNode(classes->nth(i), NotBasic, this));
}

void CgenClassTable::dfs_set_tags(CgenNodeP curr_cgen, int &curr_tag) {
    curr_cgen->set_classtag(curr_tag);
    class_tag_to_name_map_[curr_tag] = curr_cgen->get_name();
    ...
    curr_tag++;
    for (List<CgenNode> *nd = curr_cgen->get_children(); nd; nd = nd->tl()) {
        dfs_set_tags(nd->hd(), curr_tag);
    }
}

int CgenClassTable::set_subclass_cnt(CgenNodeP curr_cgen) {
    int subclass_cnt = 1;  
    for (List<CgenNode> *nd = curr_cgen->get_children(); nd; nd = nd->tl()) {
        subclass_cnt += set_subclass_cnt(nd->hd());
    }
    curr_cgen->set_subclass_cnt(subclass_cnt - 1);
    return subclass_cnt;
}
...
void CgenClassTable::install_classtags() {
    ...
    dfs_set_tags(curr_cgennode, curr_tag);
    set_subclass_cnt(curr_cgennode);  
}

void CgenClassTable::install_attrs_and_methods() {
    CgenNodeP curr_cgennode;
    Features curr_features;
    // class_local_attr_map_ and class_local_method_map_
    for (List<CgenNode> *nd = nds; nd; nd = nd->tl()) { 
        curr_cgennode = nd->hd();
        ...
        for (int i = curr_features->first(); curr_features->more(i); i = curr_features->next(i)) {
            ...
            if (curr_feature->is_method()) {
                class_local_method_map_[curr_cgen_name].push_back(static_cast<method_class *>(curr_feature));
            } else {
                class_local_attr_map_[curr_cgennode->get_name()].push_back(static_cast<attr_class *>(curr_feature));
            }
        }
    }
    // meth_offset_map
    for (List<CgenNode> *nd = nds; nd; nd = nd->tl()) {
        ...
        auto parents_list = curr_cgennode->get_parents_list();
        for (auto parent : parents_list) {
            auto local_methods = class_local_method_map_[parent->get_name()];
            for (auto method : local_methods) { 
                Symbol meth_name = method->get_name();

                if (method_offset_map_[curr_name].find(meth_name) == method_offset_map_[curr_name].end()) {
                    dispatch_tab_map_[curr_name].push_back({parent->get_name(), meth_name});
                    method_offset_map_[curr_name][meth_name] = dispatch_tab_map_[curr_name].size() - 1;
                } else {
                    auto meth_offset = method_offset_map_[curr_name][meth_name];
                    dispatch_tab_map_[curr_name][meth_offset] = {parent->get_name(), meth_name};
                }
            }
        }
        curr_cgennode->set_chain_depth(parents_list.size());
        parent_chain_map_[curr_name] = std::move(parents_list);
    }
}
...
void CgenClassTable::code_class_nametabs() {
    ...
    int len = class_tag_to_name_map_.size();

    StringEntry *str_entry;
    for (int i = 0; i < len; i++) {
        Symbol name = class_tag_to_name_map_[i];
...
bool CgenClassTable::get_meth_offset(Symbol cls, Symbol meth, int *offset) {
    if (method_offset_map_.find(cls) == method_offset_map_.end()) {
        return false;
    }
    auto find_meth = method_offset_map_[cls].find(meth);
    if (find_meth != method_offset_map_[cls].end()) {
...
void CgenClassTable::code_protobjs() {
    ...
    for (List<CgenNode> *nd = nds; nd; nd = nd->tl()) {
        ...
        // attr count
        int attr_cnt = class_local_attr_map_[curr_name].size();
        ...
        for (auto parent : parents) {
            ...
            attr_cnt += class_local_attr_map_[parent_name].size();
        }
        ...
        for (auto parent : parents) {
            Symbol parent_name = parent->get_name();
            const attrList &curr_attr_list = class_local_attr_map_[parent_name];
...
void CgenClassTable::code_object_inits() {
    ...
    for (List<CgenNode> *nd = nds; nd; nd = nd->tl()) {
        ...
        const attrList &curr_attrs = class_local_attr_map_[curr_cgen->get_name()];
...
void CgenClassTable::code_methods() {
    ...
    for (List<CgenNode> *nd = nds; nd; nd = nd->tl()) {
        ...
        if (is_basic_class(cgenname)) {  
            continue;
        }
        
        const methodList &methods = class_local_method_map_[cgenname];
        ...
        for (auto method : methods) {
            ...
            for (auto formal : formal_list) {
                env->add_formal_id(formal->get_name());
            }
...
void loop_class::code(ostream &s) {
    ...
    pred->code(s);
    emit_load(T1, ATTR_BASE_OFFSET, ACC, s);
    emit_beq(T1, ZERO, end_lebal, s); 

    body->code(s);
    emit_branch(start_lebal, s);   
    emit_label_def(end_lebal, s);  
...
void typcase_class::code(ostream &s) {
    ...
    for (auto case_class : sorted_cases) {
        ...
        int end_tag = start_tag + cgen->get_subclass_cnt();
        emit_blti(T1, start_tag, next_case_lebal, s);
        emit_bgti(T1, end_tag, next_case_lebal, s);
        
        emit_push(ACC, s);
        env->enterscope();
        env->add_local_id(case_class->get_name());
...
void eq_class::code(ostream &s) {
    ...
    if ((e1type == Int || e1type == Bool || e1type == Str) && (e2type == Int || e2type == Bool || e2type == Str)) {
        emit_load_bool(ACC, truebool, s);
