...
void CgenClassTable::generate_objTab() {
    ...
    for (int i = 0; i < tag_to_class.size(); ++i) {
        str << WORD;
        emit_protobj_ref(tag_to_class[i], str);
        str << endl;
        str << WORD;
        emit_init_ref(tag_to_class[i], str);
        str << endl;
    }
}
...
void CgenClassTable::generate_protObj() {
    for (List<CgenNode> *l = nds; l; l = l->tl()) {
        ...
        emit_protobj_ref(cur->get_name(), str);
        str << LABEL;
        ...
        if (cur->get_name() == Int || cur->get_name() == Bool) {
            ...
            // dispatch table
            str << WORD;
            emit_disptable_ref(cur->get_name(), str);
            str << endl;
...
void CgenClassTable::prepare_class_init() {
    for (List<CgenNode> *l = nds; l; l = l->tl()) {
        ...
        emit_init_ref(cur_class->get_name(), str);
        str << LABEL;
        // save register
...
void CgenClassTable::generate_method_code() {
    for (List<CgenNode> *l = nds; l; l = l->tl()) {
        ...
        for (int i = features->first(); features->more(i); i = features->next(i)) {
            Feature feature = features->nth(i);
            if (feature->getType() == FeatureType::METHOD_) {
                emit_method_ref(cur_class->get_name(), feature->get_name(), str);
                str << LABEL;
